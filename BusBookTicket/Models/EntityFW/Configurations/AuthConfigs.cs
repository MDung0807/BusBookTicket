...
namespace BusBookTicket.Models.EntityFW.Configurations
{
    public class AccountConfigs : IEntityTypeConfiguration<Account>
    {
        public void Configure(EntityTypeBuilder<Account> builder)
        {
            ...
            builder.HasOne(x => x.customer).WithOne(b => b.account).HasForeignKey<Customer>(x=> x.accountID);
            builder.HasOne(x => x.company).WithOne(builder => builder.account).HasForeignKey<Company>(x=> x.companyID);
        }
    }
}
